...
An end-to-end data analytics project that explores the shopping behavior of **3,900 retail customers**. The project follows a complete data lifecycle — cleaning and feature engineering in **Python (pandas)**, business analysis in **MySQL**, interactive reporting in **Power BI**, and a final **presentation** summarizing insights and recommendations.
...
| **Presentation (PDF)** | Final summary of findings and recommendations |
...
| File | Description |
|------|-------------|
| `raw_customer_shopping_behavior.csv` | Source dataset — 3,900 rows, 18 columns (age, gender, item, category, purchase amount, location, size, color, season, review rating, subscription, shipping, discounts, payment method, purchase frequency, etc.) |
| `customer_shopping_behavior.ipynb` | Python/pandas notebook: cleaning, feature engineering, and automated push to MySQL |
| `cleaned_customer_shopping_behavior.csv` | Cleaned dataset with standardized column names and derived columns |
| `customer_shopping_behavior_analysis.sql` | 10 business-question SQL queries |
| `customer_behavior_dashboard.pbix` | Power BI dashboard file |
| `Customer Shopping Behavior Analysis.pdf` | Written report / presentation of the analysis |

---

## How to Run the Project

The project is designed to run in four sequential stages. Follow them in order.
...
This stage:
- Loads `raw_customer_shopping_behavior.csv` with `pandas`.
...
Then open MySQL Workbench and import the file using the **Table Data Import Wizard** (right-click a schema → *Table Data Import Wizard* → select `cleaned_customer_shopping_behavior.csv`). Simple, no extra setup — good for a one-off load.
...
```sql
SOURCE customer_shopping_behavior_analysis.sql;
```
...
Open `customer_behavior_dashboard.pbix` in Power BI Desktop. Point the data source to either the cleaned CSV or the MySQL table, then refresh to explore the interactive visualizations (revenue breakdowns, customer segments, product performance, and more).

### Step 4 — Presentation

Review `Customer Shopping Behavior Analysis.pdf` for the final presentation — a summary of the key findings from the SQL analysis and Power BI dashboard, along with business recommendations.
